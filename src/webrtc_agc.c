...
webrtc_agc_set_config(int targetLevelDbfs, int compressionGaindB, int limiterEnable) {
    if (mHandle != NULL) {
        ...
        int ret = WebRtcAgc_set_config(mHandle, agcConfig);
        if (ret != 0) {
            return ret;
        }
        return 0;
...
FFI_PLUGIN_EXPORT int webrtc_agc_process(int16_t *src_audio_data, int64_t length) {
    if (mHandle != NULL) {
        ...
        for (int i = 0; i < nTotal; i++) {
            inMicLevel = 0;
            int nAgcRet = WebRtcAgc_Process(mHandle, (const int16_t *const *) &input, num_bands, samples,
